 ...
 * 
 * Mô tả lớp : viết các hàm truy vấn  dữ liệu từ database hóa đơn 
 */
package dao;
...
public class HoaDonDAO {
	...
	public double getDoanhThuNgay(Date date, String maNV) {
		...
		try {
			...
			String sql = "select sum(tongTien) as doanhThu from HoaDon where ngayLapHD = '"+date+"' and maNhanVien='"+maNV+"'";
	...
	public List<HoaDon> timHDTheoNgay_NVBH(Date from, Date to , String maNV) {
		...
		try {
			...
			Connection con = ConnectDB.getConnetction();
			String sql = "SELECT * FROM   HoaDon where ngayLapHD between '"+from+"' and '"+to+"' and  maNhanVien = '"+maNV+"'";
	...
	public List<HoaDon> timHDTheoSdtKhachHang_NVBH(JTextField txtTimKiem, String maNV) {
		...
		try {
			...
			String sql = "SELECT HoaDon.* FROM   HoaDon INNER JOIN KhachHang ON"
					+ " HoaDon.maKhachHang = KhachHang.maKhachHang where KhachHang.sdt = '"+txtTimKiem.getText()+"' and  HoaDon.maNhanVien ='"+maNV+"'";
